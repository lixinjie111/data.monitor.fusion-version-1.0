    ...
    export default {
        data() {
            return {
                ...
                <li>
                    <span class="overview-sign perception-sign"></span>
                    <span class="fusion-font">车辆感知：车辆 {{sideCar.veh || 0}}，行人 {{sideCar.person || 0}}，<br/>&nbsp;&nbsp;&nbsp;&nbsp;非机动车 {{sideCar.noMotor || 0}}</span>
                    <!--<span class="fusion-font">车辆感知：车辆 </span>-->
                </li>
            </ul>
            <p class="c-title">交通数据</p>
    ...
    export default {
        ...
        watch:{
           ...
           /*vehData:{
               handler:function (val,oldVal) {
                   let vehCount=0;
                   this.vehData.sideCar.forEach(item=>{
                       if(item.devType==1){
                           this.platformData.veh++;
                       }
                       if(item.devType==2){
                           this.v2xData.veh++;
                       }
                   });
                   this.fusionData.platVeh = this.platformData.veh+this.v2xData.veh;
               },
               deep:true
           },
           perceptionData:{
                handler: function (val, oldVal) {
                    this.fusionData.perVeh = perceptionData['veh'];
                    this.fusionData.nonMotor = perceptionData['noMotor'];
                    this.fusionData.person = perceptionData['person'];
                },
                deep:true
            }*/
        },
